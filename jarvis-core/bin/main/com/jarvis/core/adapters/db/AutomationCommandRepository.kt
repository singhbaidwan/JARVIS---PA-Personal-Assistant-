...
interface AutomationCommandRepository {
    ...
    fun recoverStaleClaims(staleBefore: Instant, now: Instant): Int
...
class SqliteAutomationCommandRepository(
    ...
    override fun recoverStaleClaims(staleBefore: Instant, now: Instant): Int {
        val staleBeforeValue = staleBefore.toString()
        val nowValue = now.toString()
        val staleError = "Command lease expired before completion was reported"

        val requeued = jdbcTemplate.update(
            """
                UPDATE automation_commands
                SET status = ?,
                    scheduled_at = ?,
        ...
        val failed = jdbcTemplate.update(
            """
                UPDATE automation_commands
                SET status = ?,
                    last_error = ?,
